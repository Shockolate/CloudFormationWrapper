module CloudFormationWrapper
  class StackManager
    def self.deploy(options)
      ...
      unless options_with_defaults[:name] && (options_with_defaults[:name].is_a? String)
      ...
      cf_client = options_with_defaults[:client] || Aws::CloudFormation::Client.new(credentials: credentials, region: region)
      
      deploy_stack(options_with_defaults[:parameters], options_with_defaults[:name], options_with_defaults[:template_path], options_with_defaults[:wait_for_stack], cf_client)
    end

    private

    def deploy_stack(parameters, stack_name, template_path, cf_client, wait)
    ...
    def list_changes(change_set_id, cf_client)
      ...
      response.changes.each do |change|
        ...
        puts "\t#{resource_change.action} - #{resource_change.logical_resource_id} aka #{resource_change.physical_resource_id} (#{resource_change.resource_type})"
        puts "\t\tScope: #{resource_change.scope}"
        puts "\t\tReplacment: #{resource_change.replacement}"
        puts "\t\tDetails:"
        resource_change.details.each do |detail|
          puts "\t\t\tTarget: #{detail.target.attribute} - #{detail.target.name} - recreate:#{detail.target.requires_recreation}"
    ...
    def wait_for_stack_to_complete(stack_name, minimum_timestamp_for_events, cf_client)
      ...
      puts "#{'-'.center(timestamp_width, '-')} #{'-'.center(logical_resource_width, '-')} #{'-'.center(resource_status_width, '-')}"
      stack = {}
      loop do
        sleep(polling_period)
        ...
        events.reverse_each do |event|
          ...
          line << event.resource_status_reason if !event.resource_status.end_with?('IN_PROGRESS') && !event.resource_status_reason.nil?
          puts line
        end
